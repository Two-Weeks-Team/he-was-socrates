...
> **Prize structure (verified 2026-05-11)**: $200K total across meta-categories **General / Impact-focused / Technical**, mapped onto 5 problem-area tracks (Education / Health / Digital Equity / Global Resilience / Safety). Confirmed sponsor prizes: **$10,000 Unsloth prize** for the best *fine-tuned* Gemma 4 model built with Unsloth, plus special mentions for Ollama. No "Google AI Edge" sponsor prize is confirmed for Gemma 4 (the Gemma 3n challenge had one; do not claim it for Gemma 4). → Out Loud's primary fit: **Digital Equity & Inclusivity** (problem-area track), which sits under **Impact-focused** (meta-category). Out Loud uses E4B as-is, so it is *not* eligible for the Unsloth prize unless the team adds a small Unsloth-built LoRA (e.g. to sharpen the document-parsing-discipline + the refusal gate) — optional, adds D-8 cost, only worth it if the team wants a second shot at a sponsor prize.

1. **Lock the track**: Digital Equity & Inclusivity (problem area) → Impact-focused (meta-category). Keep "Safety & Trust" and "Health & Sciences" as supporting framings in the writeup prose, not as the selected track.
